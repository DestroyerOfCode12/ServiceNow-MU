...
import { TOPIC_SOURCES } from "./seed-data/topic-sources";
import { FLASHCARDS } from "./seed-data/flashcards";
import { SUPPLEMENTAL_QUESTIONS } from "./seed-data/supplemental-questions";
import rawImportedQuestions from "./seed-data/imported-questions.raw.json";
import { mapQuestionToTopicSlug } from "../src/lib/import/topic-mapper";
import { estimateDifficulty } from "../src/lib/import/difficulty-heuristic";
import { computeQualityScore, EXAM_ELIGIBLE_THRESHOLD, EXAM_PREFERRED_THRESHOLD } from "../src/lib/validation/quality-score";
...
async function main() {
  console.log("== Seeding ServiceNow CSA Prep Platform ==\n");
  ...
  console.log(`Demo user ready: ${demo.email} (password: Demo1234!)\n`);
  ...
  console.log(`Blueprint version: ${blueprint.version} (${blueprint.status})`);
  ...
  console.log(`Seeded ${DOMAINS.length} domains, ${topicIdBySlug.size} topics.\n`);
  ...
  for (const src of TOPIC_SOURCES) {
    ...
    if (!check.accepted) {
      console.warn(`  SKIPPED non-allowlisted source for ${src.topicSlug}: ${src.url} (${check.reason})`);
      continue;
    }
    ...
    await prisma.topic.update({
      ...
      data: {
        ...
        validationNotes:
          src.validationNotes ??
          `Verified against official ServiceNow documentation: "${src.title}" (${src.url}).`,
      },
    });
  ...
  console.log(`Validated ${verifiedTopicSlugs.size}/${topicIdBySlug.size} topics against real official ServiceNow sources.\n`);
  ...
  for (const q of raw) {
    ...
    const topicState = {
      ...
      hasConflict: topicSlug === "csdm", // the one documented source conflict from the validation pass
    };
  ...
  console.log(`Imported ${importedCount} questions from the study guide.`);
  ...
  console.log(`Seeded ${supplementalCount} original supplemental questions (Domain 1 & 2 coverage).\n`);
  ...
  for (const row of byStatus) console.log(`  ${row.validationStatus}: ${row._count}`);
  console.log(`Exam-eligible (quality >= ${EXAM_ELIGIBLE_THRESHOLD}): ${eligible}`);
}
